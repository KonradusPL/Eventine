...
class MainPresenter<V: MainMvp.View>: BasePresenter<V>(),MainMvp.Presenter<V> {
    ...
    override fun onAttach(mvpView: V) {
        ...
        //view.changeProfileIcon(repo.preferences.getUserProfileImage())
        ...
        //Log.d("token",token)
            compositeDisposable.add(repo.rest.networkService.updateNotifToken(repo.preferences.getUserToken(),request)
    ...
    override fun sendNotifToken(token: String) {
        ...
        repo.rest.networkService.updateNotifToken(repo.preferences.getUserToken(),request)
                .subscribeOn(SchedulerProvider.io())
                .observeOn(SchedulerProvider.ui())
                .subscribe({t: String? ->
                    Log.d("tokenik",t)
